...
public class NewCompleteTree<K extends Comparable<K>, J> {
    private int size;
    private Node[] array;
    private Node root;
    private double[] temp;
    ...
    public NewCompleteTree(int capacity) {
        array = (Node[]) new Object[capacity];
        temp = new double[capacity];
        size = 0;
    }

    public void insert(K priority, J capacity) {
        Node node = new Node(priority, capacity);//Ends  con el array[0] con el valor de pivot
        if (size == 0) {
            array[1] = node;
            size++;
            return;
        ...
        for (array[0] = node; array[0].priority.compareTo(array[placement / 2].priority) < 0; placement /= 2)
